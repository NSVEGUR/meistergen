...
const applyService = catchAsync(async function (req: Request, res: Response, next: NextFunction) {
  ...
  if (!newFiles) return next(new AppError('Error in creating files', 400));
...
const getAvailable = catchAsync(async function (req: Request, res: Response, next: NextFunction) {
  ...
  const filteredServices = services.filter((service) => !serviceRequests.includes(service.id));
  const availableServices = filteredServices.map((service) => prismaExclude(service, ['id']));
  res.status(200).json({
    status: 200,
...
const getAll = catchAsync(async function (req: Request, res: Response, next: NextFunction) {
  ...
  const approvedIds = serviceRequests.map((request) => request.service.id);
  const requests = serviceRequests.map((request) => {
    return {
      uid: request.uid,
      service: prismaExclude(request.service, ['id'])
  ...
  const services = await prisma.service.findMany();
  const filteredServices = services.filter((service) => !approvedIds.includes(service.id));
  const availableServices = filteredServices.map((service) => prismaExclude(service, ['id']));
  res.status(200).json({
    status: 200,
...
const getService = catchAsync(async function (req: Request, res: Response, next: NextFunction) {
